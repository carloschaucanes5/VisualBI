<%-- 
    Document   : dwhQueryFilter
    Created on : 13/11/2014, 11:40:04 AM
    Author     : Carlitos
--%>

<%@page import="Dwh.Table.QueryTable"%>
<%@page import="Dwh.Connection.ClassConnection"%>
<%@page import="Dwh.Cube.FilterQuery"%>
...
<html>
    ...
    <body>
        <%
       try
        {   //out.println("dwhQueryFilter.jsp");   
            String sql = "";
            ...
            ClassConnection cc = new ClassConnection(vecCon[0].trim(), vecCon[1].trim(),vecCon[2].trim() , vecCon[3].trim(), vecCon[4].trim(), vecCon[5].trim());
                if(cc.processConnection() == true)
                {
                  ...
                  {
                      //out.println(dim1+dim2+dim3);
                      FilterQuery fq = new FilterQuery(d1[0], d2[0], d3[0], d1[1], d2[1], d3[1], sc.getNameCube(), measures);
        ...
        %>
        <!--div id="dialog-messagea" title="Information visulaBITools"></div-->
    </body>
</html>
